...
export default function PageOrb(){
 ...
 useEffect(()=>{const el=mount.current,scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(38,el.clientWidth/el.clientHeight,.1,50);camera.position.z=8.4;
  const renderer=new THREE.WebGLRenderer({alpha:true,antialias:innerWidth>700,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<800?1:1.2));renderer.setSize(el.clientWidth,el.clientHeight);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.2;el.appendChild(renderer.domElement);
  ...
  let raf,visible=true,running=false;const pointer={x:0,y:0},move=e=>{pointer.x=e.clientX/innerWidth-.5;pointer.y=e.clientY/innerHeight-.5},resize=()=>{camera.aspect=el.clientWidth/el.clientHeight;camera.updateProjectionMatrix();renderer.setSize(el.clientWidth,el.clientHeight)};addEventListener('pointermove',move,{passive:true});addEventListener('resize',resize);const clock=new THREE.Clock();function draw(){if(!running)return;const t=clock.getElapsedTime();group.rotation.y+=(pointer.x*.55+t*.035-group.rotation.y)*.018;group.rotation.x+=(-pointer.y*.28+.1-group.rotation.x)*.022;group.rotation.z=Math.sin(t*.28)*.08;group.position.y=Math.sin(t*.62)*.1;renderer.render(scene,camera);raf=requestAnimationFrame(draw)}const sync=()=>{const next=visible&&!document.hidden;if(next&&!running){running=true;clock.start();draw()}else if(!next&&running){running=false;cancelAnimationFrame(raf)}};const observer=new IntersectionObserver(([entry])=>{visible=entry.isIntersecting;sync()},{rootMargin:'10%'});observer.observe(el);document.addEventListener('visibilitychange',sync);sync();return()=>{running=false;cancelAnimationFrame(raf);observer.disconnect();document.removeEventListener('visibilitychange',sync);removeEventListener('pointermove',move);removeEventListener('resize',resize);group.traverse(object=>{object.geometry?.dispose();if(Array.isArray(object.material))object.material.forEach(m=>m.dispose());else object.material?.dispose()});renderer.dispose();el.removeChild(renderer.domElement)}},[path]);
