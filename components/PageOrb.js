...
export default function PageOrb(){
 ...
 const group=new THREE.Group();group.position.x=.7;scene.add(group);const glass=new THREE.MeshPhysicalMaterial({color:0x9bd8d1,transmission:.55,thickness:1.7,roughness:.08,metalness:.12,clearcoat:1,ior:1.42,iridescence:.45});const blob=new THREE.Mesh(new THREE.TorusKnotGeometry(1.25,.48,150,28,2,3),glass);group.add(blob);
