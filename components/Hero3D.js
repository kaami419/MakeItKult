...
export default function Hero3D() {
  ...
  useEffect(() => {
    ...
    const count=550, positions=new Float32Array(count*3);
    ...
    function animate(){const t=clock.getElapsedTime();group.rotation.y+=(pointer.x*.35-group.rotation.y)*.025;group.rotation.x+=(-pointer.y*.22-group.rotation.x)*.025;knot.rotation.z=t*.09;wire.rotation.y=-t*.06;wire.rotation.x=t*.035;group.position.y=Math.sin(t*.7)*.08;renderer.render(scene,camera);raf=requestAnimationFrame(animate)} animate();
    return()=>{cancelAnimationFrame(raf);window.removeEventListener('pointermove',move);window.removeEventListener('resize',resize);geometry.dispose();material.dispose();renderer.dispose();el.removeChild(renderer.domElement)};
  },[]);
